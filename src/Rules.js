...
class Rule {
  ...
  freq(dice) {
    ...
    const freqs = new Map();
    for (let d of dice) freqs.set(d, (!freqs.get(d)) ? 1 : freqs.get(d) + 1);
...
class FullHouse extends Rule {
  ...
  evalRoll = dice => {
    const newdice = new Map();

    for (let sub of dice) newdice.set(sub, (newdice.get(sub) || 0) + 1)

    const obj = Array.from(newdice.values())
    console.log(obj)
    return (obj[0] + obj[1] === 5) ? this.score : 0
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    ...
    if (diceSet.has(2) && diceSet.has(3) && diceSet.has(4) && (diceSet.has(1) || diceSet.has(5)))
      return this.score
    if (diceSet.has(5) && diceSet.has(3) && diceSet.has(4) && (diceSet.has(2) || diceSet.has(6)))
      return this.score
